...
public class MethodRuntime extends ParameterRuntime implements IBase2 {
    ...
    public String generatePlantUML() {
        String plantUML = getTabs(1) + access.getPlantUML() + getReservedPlantUML(isAbstract, ABSTRACT) + getReservedPlantUML(isStatic, STATIC) + getName();
        plantUML = plantUML + "(" + generatePlantUML(parameters) + ")" + " : " + getClas().getName() + generateParamClass(getClas()) + generateArray();
        return plantUML;
    }

    @Override
    public String generate() {
        String text = getTabs(1) + access.getText() + " " + getReservedCode(isAbstract, isFinal(), isStatic) + getClas().getName()
                + generateParamClass(getClas()) + generateArray() + " " + getName() + "(";
