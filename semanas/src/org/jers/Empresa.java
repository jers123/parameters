...
public class Empresa {
    ...
    private boolean esUltimoDiaFebrero(LocalDate fecha) {
        return fecha.getMonth() == Month.FEBRUARY &&
                (fecha.getDayOfMonth() == 28 || fecha.getDayOfMonth() == 29);
    }

    // Verifica si un mes tiene 31 días
    private boolean tiene31Dias(Month mes) {
        return mes == Month.JANUARY || mes == Month.MARCH || mes == Month.MAY ||
                mes == Month.JULY || mes == Month.AUGUST || mes == Month.OCTOBER || mes == Month.DECEMBER;
    }
}